...
private _dragParasite = [0, 0, 0];
{
    ...
    _dragParasite set [_velIndex, _force * _massStandard * _densityRatio * (awesome_aerodynamics_dragSourceMultiplier select 0)];
} forEach [[0, 0], [1, 2], [2, 1]];
...
if (_liftVector isEqualType []) then {
    ...
    _dragInduced = (vectorNormalized _airVel) vectorMultiply (_inducedConst * _inducedValue * (awesome_aerodynamics_dragSourceMultiplier select 1));
};

// wave drag (zero lift drag)
awesome_aerodynamics_waveCdArray params ["_machCritical", "_transStart", "_machMaxCd", "_transEnd", "_multiplierMax", "_roundFwd", "_roundAft", "_machPower"];
...
private _dragWave = [0, 0, 0];
{
    ...
    _dragWave set [_velIndex, _force * _massStandard * _densityRatio * (awesome_aerodynamics_dragSourceMultiplier select 2)];
} forEach [[0, 0], [1, 2], [2, 1]];
...
// diag_log format ["awesome_aerodynamics _dragParasite: %1, _dragInduced: %2, _dragForceEnhanced: %3", _dragParasite, _dragInduced, _dragForceEnhanced];
if (AWESOME_DEVMODE_LOG) then {
    diag_log format ["awesome_aerodynamics Mach: %1, Base: %2, Parasite: %3, Induced: %4, Wave: %5, Total: %6", _machNumber, vectorMagnitude _dragParasite / (awesome_aerodynamics_dragSourceMultiplier select 0), vectorMagnitude _dragParasite, vectorMagnitude _dragInduced, vectorMagnitude _dragWave, vectorMagnitude _dragForceEnhanced];
};
