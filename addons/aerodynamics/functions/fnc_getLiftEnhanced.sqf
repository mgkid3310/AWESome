...
private _liftValue = [_liftArray, _speedMax, 1.25 / (count _liftArray - 1), _speedKPH] call awesome_aerodynamics_fnc_extractCoefArray;

private _liftForceEnhanced = (vectorNormalized _airVel) vectorCrossProduct [_liftValue * awesome_aerodynamics_liftGFactor * _massStandard * _densityRatio, 0, 0];
_liftForceEnhanced set [2, abs (_liftForceEnhanced select 2)];

// report if needed (dev script)
// diag_log format ["awesome_aerodynamics _liftForceEnhanced: %1", _liftForceEnhanced];
