...
while {((player distance _monitor) < 10) && (player getVariable ["isUsingRadarScreen", true])} do {
    ...
    if (time > _timeNext) then {
        ...
        _planeMarkers = [_planes, "b_plane"] call awesome_atc_fnc_createMarkers;
        _heliMarkers = [_helies, "b_air"] call awesome_atc_fnc_createMarkers;
        [_planeMarkers, _heliMarkers] call awesome_atc_fnc_updateMarkerSpacing;
    ...
    // ACE_map capability
    if (awesome_awesome_hasACEMap) then {
        _crewList = [];
        {
            {
