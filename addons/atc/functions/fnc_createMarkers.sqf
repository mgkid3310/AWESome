...
private _useCallsign = missionNamespace getVariable ["awesome_atc_displayCallsign", false];
private _unitSetting = missionNamespace getVariable ["awesome_atc_unitSetting", 0];
private _return = [];
{
    ...
    switch (_unitSetting) do {
        ...
        case (1): { // meter / knot
            _speed = round (awesome_awesome_kphToKnot * speed _x);
            _altitude = round (getPosASL _x select 2);
        };
        case (2): { // feet / kph
            _speed = round speed _x;
            _altitude = round (awesome_awesome_mToFt * (getPosASL _x select 2));
        };
        case (3): { // feet / knot
            _speed = round (awesome_awesome_kphToKnot * speed _x);
            _altitude = round (awesome_awesome_mToFt * (getPosASL _x select 2));
    ...
    _marker0 = createMarkerLocal [format ["awesome_atc_%1_%2_0", _type, _forEachIndex], getPos _x];
    _marker0 setMarkerTypeLocal _type;
    _marker0 setMarkerTextLocal "";

    _marker1 = createMarkerLocal [format ["awesome_atc_%1_%2_1", _type, _forEachIndex], getPos _x];
    _marker1 setMarkerTypeLocal _type;
    _marker1 setMarkerSizeLocal [0, 0];
    _marker1 setMarkerTextLocal _line1;

    _marker2 = createMarkerLocal [format ["awesome_atc_%1_%2_2", _type, _forEachIndex], getPos _x];
    _marker2 setMarkerTypeLocal _type;
    _marker2 setMarkerSizeLocal [0, 0];
    _marker2 setMarkerTextLocal _line2;

    _marker3 = createMarkerLocal [format ["awesome_atc_%1_%2_3", _type, _forEachIndex], getPos _x];
...
[_return] call awesome_atc_fnc_updateMarkerSpacing;
