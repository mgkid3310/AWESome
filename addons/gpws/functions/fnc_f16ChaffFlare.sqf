...
DEV_CHAT("awesome_gpws: f16ChaffFlare run");
params ["_vehicle", "_weapon", "_muzzle", "_mode"];

if ((_weapon in awesome_gpws_ChaffFlareList) && !(_vehicle getVariable ["CMrunnig", false])) then {
	DEV_CHAT("awesome_gpws: f16ChaffFlare active");
	...
	DEV_CHAT("awesome_gpws: f16ChaffFlare waiting");
	...
	if ((alive _vehicle) && (player in _vehicle)) then {
		switch (true) do {
			...
			case (_resultingAmmo <= 0): {
				DEV_CHAT("awesome_gpws: f16_chaffFlareOut");
				_vehicle setVariable ["orbisGPWSready", false];
				[_vehicle, "f16_chaffFlareOut"] spawn awesome_gpws_fnc_speakGPWS;
			};

			// f16_chaffFlareLow
			case ((_resultingAmmo <= (_vehicle getVariable ["lowCMcount", _resultingAmmo - 1])) && !(_vehicle getVariable ["CMlowAlerted", false])): {
				DEV_CHAT("awesome_gpws: f16_chaffFlareLow");
				_vehicle setVariable ["orbisGPWSready", false];
				[_vehicle, "f16_chaffFlareLow"] spawn awesome_gpws_fnc_speakGPWS;
				_vehicle setVariable ["CMlowAlerted", true];
			};

			// f16_chaffFlare
			default {
				DEV_CHAT("awesome_gpws: f16_chaffFlare");
				_vehicle setVariable ["orbisGPWSready", false];
				[_vehicle, "f16_chaffFlare"] spawn awesome_gpws_fnc_speakGPWS;
			};
		};
	};
