...
DEV_CHAT("awesome_gpws: getInMan run");

if !([_unit, _vehicle, 1] call awesome_awesome_fnc_isCrew) exitWith {};
...
if (_modeCurrent isEqualTo "init") then {
	private _defaultVolumeLow = missionNamespace getVariable ["awesome_gpws_defaultVolumeLow", false];
	_vehicle setVariable ["orbisGPWSvolumeLow", _defaultVolumeLow, true];

	private _defaultMode = missionNamespace getVariable ["awesome_gpws_personallDefault", "none"];
