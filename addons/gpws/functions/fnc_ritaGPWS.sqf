...
DEV_CHAT("awesome_gpws: ritaGPWS run");
private _vehicle = _this select 0;

if !((alive _vehicle) && (player in _vehicle)) exitWith {};
DEV_CHAT("awesome_gpws: ritaGPWS active");
...
DEV_CHAT("awesome_gpws: ritaGPWS variables init done");

private _frameNo = diag_frameNo;
waitUntil {(diag_frameNo > _frameNo) && (time > _timeOld)};

// rita_online
DEV_CHAT("awesome_gpws: rita_online");
_vehicle setVariable ["orbisGPWSready", false];
[_vehicle, "rita_online"] spawn awesome_gpws_fnc_speakGPWS;
...
while {(alive _vehicle) && (player in _vehicle) && (_vehicle getVariable ["orbisGPWSmodeLocal", "off"] isEqualTo "rita")} do {
	...
	_posExpect = (getPosASL _vehicle) vectorAdd (velocity _vehicle vectorMultiply awesome_gpws_ritaPullupTime);
	...
	if (_vehicle getVariable ["orbisGPWSready", true]) then {
		switch (true) do {
			...
			case ((_expectTerrainAlt > (_posExpect select 2)) && (_flightphase isEqualTo "inFlight")): {
				DEV_CHAT("awesome_gpws: rita_pullUp");
				_vehicle setVariable ["orbisGPWSready", false];
				[_vehicle, "rita_pullUp"] spawn awesome_gpws_fnc_speakGPWS;
			};

			// rita_altitude (inFlight)
			case ((_altRadar < awesome_gpws_ritaLowAltitude) && (_flightphase isEqualTo "inFlight")): {
				DEV_CHAT("awesome_gpws: rita_altitude");
				_vehicle setVariable ["orbisGPWSready", false];
				[_vehicle, "rita_altitude"] spawn awesome_gpws_fnc_speakGPWS;
			};

			// rita_overload
			case ((_cosAOA < cos awesome_gpws_ritaMaxAOA) && (speed _vehicle > 50)): {
				DEV_CHAT("awesome_gpws: rita_overload");
				_vehicle setVariable ["orbisGPWSready", false];
				[_vehicle, "rita_overload"] spawn awesome_gpws_fnc_speakGPWS;
			};

			// rita_angle (takeOff / inFlight / landing / final)
			case ((_pitchAngle < awesome_gpws_ritaMaxDive) && (_flightphase in ["takeOff", "inFlight", "landing", "final"])): {
				DEV_CHAT("awesome_gpws: rita_angle");
				_vehicle setVariable ["orbisGPWSready", false];
				[_vehicle, "rita_angle"] spawn awesome_gpws_fnc_speakGPWS;
			};

			// rita_speed
			case (_currentSpeed > _maxSpeed): {
				DEV_CHAT("awesome_gpws: rita_speed");
				_vehicle setVariable ["orbisGPWSready", false];
				[_vehicle, "rita_speed"] spawn awesome_gpws_fnc_speakGPWS;
			};
...
DEV_CHAT("awesome_gpws: ritaGPWS loop terminated");

DEV_CHAT("awesome_gpws: ritaGPWS ended");
