...
if (_vehicle isKindOf "Offroad_01_base_F") then {
	...
	_vehicle addAction [localize LSTRING(removeTowBar), {[_this select 0] call FUNC(removeTowBar)}, nil, 1, false, true, "", "(_this isEqualTo driver _target) && (_target getVariable ['orbis_ground_hasTowBarDeployed', false]) && (speed _target < 1)", 10];
};
/* if (_vehicle isKindOf "Plane") then {
	...
	_vehicle addAction ["Release Parking Brake", {[_this select 0] call FUNC(parkingBrakeRelease)}, nil, 1, false, true, "", "([_this, _target, 1] call orbis_main_fnc_isCrew) && (_target getVariable ['orbis_ground_parkingBrakeSet', false])", 10];
}; */
