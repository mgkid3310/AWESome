...
if (!(_car getVariable ["awesome_hasTowBarDeployed", false]) || (_car getVariable ["awesome_isTowingPlane", false])) exitWith {
...
private _checkStart = getArray (configFile >> "CfgVehicles" >> (typeOf _car) >> "awesome_towBarCheckStart");
private _checkEnd = getArray (configFile >> "CfgVehicles" >> (typeOf _car) >> "awesome_towBarCheckEnd");
...
if (isNull _target) then {
    _towBar = _car getVariable ["awesome_towBarObject", objNull];
