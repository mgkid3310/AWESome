...
_vehicle addAction ["Set to Betty (F-16)", "['orbisStartGPWS', [_this select 0, 'f16']] call CBA_fnc_globalEvent", nil, 1, false, true, "", "(isClass (configFile >> 'CfgPatches' >> 'orbis_gpws')) && (_this in [driver _target, gunner _target, commander _target]) && (_target getVariable ['orbisGPWSenabled', false]) && (_target getVariable ['orbisGPWSmode', ''] != 'f16')", 10];
...
_vehicle addAction ["Set to B747 GPWS", "['orbisStartGPWS', [_this select 0, 'b747']] call CBA_fnc_globalEvent", nil, 1, false, true, "", "(isClass (configFile >> 'CfgPatches' >> 'orbis_gpws')) && (_this in [driver _target, gunner _target, commander _target]) && (_target getVariable ['orbisGPWSenabled', false]) && (_target getVariable ['orbisGPWSmode', ''] != 'b747')", 10];
